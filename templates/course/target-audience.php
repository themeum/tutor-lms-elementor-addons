	...
	<ul class="etlms-course-widget-list-items tutor-fs-6 tutor-color-black">
		...
		<?php foreach ($target_audience as $audience): ?>
			<li class="etlms-course-widget-list-item">
				...
				<?php 
					if ( isset( $settings['course_target_audience_list_icon']['library'] ) && 'svg' === $settings['course_target_audience_list_icon']['library'] ) {
						\Elementor\Icons_Manager::render_icon( $settings['course_target_audience_list_icon'], array( 'aria-hidden' => 'true' ) );
					} else {
						?>
							<i aria-hidden="true" class="<?php echo esc_attr( $settings['course_target_audience_list_icon']['value'] ); ?>"></i>
						<?php
					}
				?>	
				</span>
